...
case class ActivityPattern(task: CapabilityTask) extends WorkflowPattern
...
class SolutionPattern(sol:AbstractWorkflow) {

  val start_event = sol.items.filter(i => i.isInstanceOf[StartEvent]).head
  var visited: List[AbstractWorkflowItem] = List(StartEvent(0,s"start_${0}"))

  var first_node: AbstractWorkflowItem = sol.successors(start_event).head
  ...
  private def node_to_graphviz(node: WorkflowPattern, root_node : String, label:String=""):String = {
    ...
    node match {
      ...
      case ActivityPattern(task) =>
        string += "\""+task.cap.id+"\";\n"
        string += "\""+root_node+"\" -> \""+task.cap.id+"\"[label=\""+label+"\"];\n"

    }
    string
  }

  private def decompose(pivot_node: AbstractWorkflowItem, visited: List[AbstractWorkflowItem], inside : String): (Option[WorkflowPattern], AbstractWorkflowItem, List[AbstractWorkflowItem]) = {
  ...
  private def expore_if_choice(pivot_node: SplitGateway, visited: List[AbstractWorkflowItem]): (Option[WorkflowPattern], AbstractWorkflowItem, List[AbstractWorkflowItem]) = {
    var branches :List[WorkflowPattern]=List.empty
    var ending_points : Set[AbstractWorkflowItem]=Set.empty
  ...
  private def explore_if_loop(pivot_node: JoinGateway,visited: List[AbstractWorkflowItem]): (Option[WorkflowPattern], AbstractWorkflowItem, List[AbstractWorkflowItem]) = {
    ...
    /* forward */
    var exit_split : Set[AbstractWorkflowItem]=Set.empty
    var error: Boolean = false
    for (i<-split_successors if !error) {
