...
trait LTLFormulaParserTrait extends JavaTokenParsers with TermParserTrait  {
  ...
  def ltl_formula : Parser[LogicFormula with LTLNature] = ltl_exist_formula | ltl_foreach_formula | ltl_until_formula //| ltl_comma_formula
  ...
  def ltl_until_formula : Parser[LogicFormula with LTLNature] = ltl_left_formula~"U"~ltl_formula ^^ {case form1~_~form2 => b.until(form1,form2)} | ltl_release_formula ^^ { x=>x}
  def ltl_release_formula : Parser[LogicFormula with LTLNature] = ltl_left_formula~"R"~ltl_formula ^^ {case form1~_~form2 => b.release(form1,form2)} | ltl_and_formula ^^ { x=>x}

  def ltl_and_formula : Parser[LogicFormula with LTLNature] = ltl_left_formula~"and"~ltl_formula ^^ {case form1~_~form2 => b.and(form1,form2)} | ltl_or_formula ^^ { x=>x}
  def ltl_or_formula : Parser[LogicFormula with LTLNature] = ltl_left_formula~"or"~ltl_formula ^^ {case form1~_~form2 => b.or(form1,form2)} | ltl_impl_formula ^^ { x=>x}
  def ltl_impl_formula : Parser[LogicFormula with LTLNature] = ltl_left_formula~"->"~ltl_formula ^^ {case form1~_~form2 => b.implies(form1,form2)} | ltl_biiml_formula ^^ { x=>x}
  def ltl_biiml_formula : Parser[LogicFormula with LTLNature] = ltl_left_formula~"<->"~ltl_formula ^^ {case form1~_~form2 => b.biimpl(form1,form2)} | ltl_not_formula ^^ { x=>x}


  def ltl_not_formula : Parser[LogicFormula with LTLNature] = "not"~ltl_formula ^^ {case _~form =>b.not(form) } | ltl_comma_formula | ltl_left_formula
  def ltl_comma_formula : Parser[LogicFormula with LTLNature] = "("~ltl_formula~")" ^^ {case _~form~_ => form}

  def ltl_left_formula : Parser[LogicFormula with LTLNature] = ltl_unary_temporal_formula  | "true" ^^ { _=>b.truth} | "false" ^^ { _=>b.falsity} | ltl_predicate
