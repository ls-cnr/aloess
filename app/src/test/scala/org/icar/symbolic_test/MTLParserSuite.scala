...
class MTLParserSuite extends AnyFunSuite {
  ...
  test("parsing until test1 U[1,3] test2") {
    val parser = new MTLFormulaParser
    val b = new MTLBuilder
    val result : parser.ParseResult[LogicFormula with MTLNature] = parser.parseAll(parser.mtl_formula,"test1 U[1,3] test2")
    assert(result.successful)
    val f : LogicFormula with MTLNature = result.get
  ...
  test("parsing until/conjunction test1 U[3,4] (test2 and test3)") {
    val parser = new MTLFormulaParser
    val b = new MTLBuilder
    val result : parser.ParseResult[LogicFormula with MTLNature] = parser.parseAll(parser.mtl_formula,"test1 U[3,4] (test2 and test3)")
    assert(result.successful)
    val f : LogicFormula with MTLNature = result.get
  ...
  test("parsing gobally/conjunction G[0,1] (test2 and test3)") {
    val parser = new MTLFormulaParser
    val b = new MTLBuilder
    val result : parser.ParseResult[LogicFormula with MTLNature] = parser.parseAll(parser.mtl_formula,"G[0,1] (test2 and test3)")
    assert(result.successful)
    val f : LogicFormula with MTLNature = result.get
  ...
  test("parsing finally/disjunction F[0,111] (test2 or test3)") {
    val parser = new MTLFormulaParser
    val b = new MTLBuilder
    val result : parser.ParseResult[LogicFormula with MTLNature] = parser.parseAll(parser.mtl_formula,"F[0,111] (test2 or test3)")
    assert(result.successful)
    val f : LogicFormula with MTLNature = result.get
  ...
  test("parsing nested globally finally F[0,111] G[0,111] a") {
    val parser = new MTLFormulaParser
    val b = new MTLBuilder
    val result : parser.ParseResult[LogicFormula with MTLNature] = parser.parseAll(parser.mtl_formula,"F[0,111] G[0,111] a")
    assert(result.successful)
    val f : LogicFormula with MTLNature = result.get
