...
class MTLFormulaSuite extends AnyFunSuite {
  test("building MTL formula without temporal operators") {
    ...
    assert(formula == Implication( Proposition("test",List()),UnivQuantifier(VariableTerm("integer"),Predicate("test",List(VariableTerm("integer"))))))
  }
  ...
  test("building MTL formula with Metric Finally") {
    ...
    assert(formula == MetricFinally(Conjunction(List(Proposition("test",List()),UnivQuantifier(VariableTerm("integer"),Predicate("test",List(VariableTerm("integer")))))),MetricInterval(1,5)) )
  }
